...
Syntax also matters when reusing requests. Wildcards support Elasticsearch-style `*` and `?`, while regular expressions use Rust/RE2-style syntax. Invalid regular expressions receive a structured `400` response. The [compatibility guide]({{ "/compatibility/" | relative_url }}) describes these boundaries.
