...
Syntax also matters when reusing requests. Wildcards support Elasticsearch-style `*` and `?`, while regular expressions use Rust/RE2-style syntax. Invalid regular expressions receive a structured `400` response. The [compatibility guide]({{ "/compatibility.html" | relative_url }}) describes these boundaries.

Pattern matching is a gateway-side scan. A pattern-only query scans the collection in bounded scroll pages; the page size does not bound the total work. This makes the feature useful for modest catalogues and identifier lookups, but unsuitable for unbounded, high-cardinality pattern queries. If pattern traffic becomes a primary workload, evaluate a dedicated indexed pattern field or a purpose-built search path.
