...
Each ES index maps to a Qdrant collection named `es_<safe-index-name>`. SQLite stores the original mapping and the text-field vector names so restarts do not depend on collection introspection. Documents are payload-first: the original source is stored under `_source`, while top-level fields are duplicated for Qdrant filtering.
